...
if __name__ == "__main__":
    ...
    synthetic_fica_dmap = np.concatenate([synthetic_fica_scores, synthetic_dmap_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_fica_dmap, synthetic_fica_dmap)
    ...
    plt.tight_layout()
    plt.savefig(save_path + 'fica_dmap_full_knowledge_mia_roc_curve.png', dpi=300)
    plt.close()

    # Full-Knowledge MIA: FICA + UMAP
    real_fica_umap = np.concatenate([real_fica_scores, real_umap_embedding], axis=1)
    synthetic_fica_umap = np.concatenate([synthetic_fica_scores, synthetic_umap_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_fica_umap, synthetic_fica_umap)
    print(f"The Full-Knowledge MIA AUC-ROC is: {mia_auc_roc:.4f}")
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(save_path + 'fica_umap_full_knowledge_mia_roc_curve.png', dpi=300)
    plt.close()

    # Full-Knowledge MIA: FICA + kPCA
    real_fica_kpca = np.concatenate([real_fica_scores, real_kpca_embedding], axis=1)
    synthetic_fica_kpca = np.concatenate([synthetic_fica_scores, synthetic_kpca_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_fica_kpca, synthetic_fica_kpca)
    print(f"The Full-Knowledge MIA AUC-ROC is: {mia_auc_roc:.4f}")
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(save_path + 'fica_kpca_full_knowledge_mia_roc_curve.png', dpi=300)
    plt.close()