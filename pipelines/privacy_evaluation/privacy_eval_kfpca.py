...
if __name__ == "__main__":
    ...
    synthetic_kfpca_dmap = np.concatenate([synthetic_kfpca_embedding, synthetic_dmap_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_kfpca_dmap, synthetic_kfpca_dmap)
    ...
    plt.tight_layout()
    plt.savefig(save_path + 'kfpca_dmap_full_knowledge_mia_roc_curve.png', dpi=300)
    plt.close()

    # Full-Knowledge MIA: kFPCA + UMAP
    real_kfpca_umap = np.concatenate([real_kfpca_embedding, real_umap_embedding], axis=1)
    synthetic_kfpca_umap = np.concatenate([synthetic_kfpca_embedding, synthetic_umap_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_kfpca_umap, synthetic_kfpca_umap)
    print(f"The Full-Knowledge MIA AUC-ROC is: {mia_auc_roc:.4f}")
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(save_path + 'kfpca_umap_full_knowledge_mia_roc_curve.png', dpi=300)
    plt.close()

    # Full-Knowledge MIA: kFPCA + kPCA
    real_kfpca_kpca = np.concatenate([real_kfpca_embedding, real_kpca_embedding], axis=1)
    synthetic_kfpca_kpca = np.concatenate([synthetic_kfpca_embedding, synthetic_kpca_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_kfpca_kpca, synthetic_kfpca_kpca)
    print(f"The Full-Knowledge MIA AUC-ROC is: {mia_auc_roc:.4f}")
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(save_path + 'kfpca_kpca_full_knowledge_mia_roc_curve.png', dpi=300)
    plt.close()