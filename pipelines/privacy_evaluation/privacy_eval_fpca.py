...
if __name__ == "__main__":
    ...
    # Result save path
    save_path = f"images/privacy_eval/fpca/"
    path=Path(save_path)
    ...
    # Apply Holdout FPCA on synthetic dataset
    ...
    fpc_density_ratio = domias(holdout_scores, real_scores, synthetic_scores)
    ...
    # Apply holdout diffusion map on synthetic FPC scores
    synthetic_dmap_embedding = holdout_dmap.transform(synthetic_scores)
    ...
    synthetic_fpca_dmap = np.concatenate([synthetic_scores, synthetic_dmap_embedding], axis=1)
    fpr, tpr, thresholds, mia_auc_roc = full_knowledge_mia(real_fpca_dmap, synthetic_fpca_dmap)
