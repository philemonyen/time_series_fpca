...
if __name__ == "__main__":
    ...
    # Result save path
    save_path = f"images/fidelity_val/fica/"
    path=Path(save_path)
    ...
    for scenario in scenarios:
        ...
        for key, value in datasets.items():
            ...
            synthetic_fica_scores, synthetic_fica_components, synthetic_ica = compute_fica(flaw_scores_fpca, real_components)

            ## Evaluate: Gromov Wasserstein
            fica_gw = gromov_wasserstein(real_fica_scores, flaw_scores_fpca)

            #### ------------ Shared FICA ------------ ####
            shared_flaw_scores = real_ica.transform(flaw_scores_fpca)

            # Evaluation: MMD, Mahalanobis, FIC KS, PRDC, LMR
            fica_score_mmd = mmd(real_fica_scores, shared_flaw_scores)
            fica_score_mahalanobis = mahalanobis(real_fica_scores, shared_flaw_scores)
            fica_score_ks = kolmogorov_smirnov(real_fica_scores, shared_flaw_scores)
            fica_prdc = prdc(real_fica_scores, shared_flaw_scores)
            fica_lmr = local_mixing_ratio(real_fica_scores, shared_flaw_scores)
            ...
            plt.ylabel("Score")
            plt.title(f"FICA PRDC: {scenario}, Flaw Scale: {key}")
            plt.legend()
            plt.savefig(save_path + f"FICA_PRDC_{scenario}_{key}.png")
            plt.close()
            ...
            plt.title(f"FICA LMR: {scenario}, Flaw Scale: {key}")
            plt.legend()
            plt.savefig(save_path + f"FICA_LMR_{scenario}_{key}.png")
            plt.close()
    ...
    with open(save_path + f"fidelity_val_fica_result.txt", "w") as f:
        for scenario in scenarios:
